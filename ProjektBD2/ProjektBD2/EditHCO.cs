...
namespace ProjektBD2
{
    public partial class EditHCO : Form
    {
        ...
        public EditHCO(Form parentFormReference)
        {
            InitializeComponent();
        ...
        {
            String commandText = "UPDATE HCOSet " +
                                 "SET Name=@Name, Range=@Range, Level=@Level, SpecialType=@SpecialType, " +
                                 "Beds=@Beds, Employees=@Employees, PhoneNumber=@PhoneNumber, " +
                                 "Email=@Email, Website=@Website, AddressID=@AddressID " +
                                 "WHERE HCOID=@param";
            string sConnection = Properties.Settings.Default.BD2ConnectionString;
            SqlConnection conn = new SqlConnection();
            conn.ConnectionString = sConnection;
